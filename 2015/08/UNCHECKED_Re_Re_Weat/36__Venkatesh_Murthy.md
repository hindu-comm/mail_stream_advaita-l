+++
title = "36 Venkatesh Murthy"
date = "2015-08-07"
upstream_url = "https://lists.advaita-vedanta.org/archives/advaita-l/2015-August/039310.html"

+++
[Archive link](https://lists.advaita-vedanta.org/archives/advaita-l/2015-August/039310.html)

 Namaste
The Vedas have two parts. Karma Kanda and Jnana Kanda. In Karma Kanda
they describe rituals like Yajnas. In Jnana Kanda they describe
Brahman.

When a person has realized Brahman he becomes Brahman. For him Karma
Kanda of Vedas is useless. He cannot do rituals for going to heaven.
It will be like running after mirage water when you have a bottle of
water in your hand.

He cannot have use for Jnana Kanda also because he has become Brahman.
What new knowledge can the Vedas teach him ? Therefore both parts of
Vedas will become useless for him. But he should show respect to
Vedas. Like Janaka he must respect them like a leader for other people
to follow.

On Fri, Aug 7, 2015 at 2:17 AM, Ryan Armstrong via Advaita-l
<advaita-l at lists.advaita-vedanta.org> wrote:
> Dear Srirudra
> It would seem that there is a subtle difference between the reality of the
> vedaa (as the question was posed) and the need for them..
> The need is great, and if we see how the world is these days, the need for
> the vedaa is undisputed.
> But even the immortal vedaa as well as the amurti devaa - from the point of
> complete unity - are all prapancha and hence ultimately not real.
> This ignorance is cured by vedaa - but once the malady is gone, no medicine
> is required for the one cured of delusion.
> But by grace, they set the example for us and practice study, meditation
> and even ritual.
> They alone are really able to administer the "medicine" to those still
> afflicted by mayaa.
>
> I have great need for the veda. So too, is there a need for a teacher.
> Until the illusion is completely dispelled, there is always need.
>
> But as to reality - sarvan khalvidam brahmaa
>
> Yours in Truth
> Ryan
>
>
> On 5 August 2015 at 17:59, Srirudra <srirudra at gmail.com> wrote:
>
>> Dear
>> In that case -that is the treasure map is no longer useful for the
>> treasure hunter after finding the treasure-the treasure becomes a
>> singularity to borrow from our Big bangers!So what will happen to other
>> seekers.?Just an aside.
>> Vedas are eternal and can not be deemed to be of no use even for the
>> realised one as it is breath of the Brahman or it is coequal with
>> Brahman.R.Krishnamoorthy.
>>
>> Sent from my iPad
>>
>> > On 05-Aug-2015, at 2:38 pm, Ryan Armstrong via Advaita-l <
>> advaita-l at lists.advaita-vedanta.org> wrote:
>> >
>> > Dear Harsha
>> > Namaste
...
>> > nor unconscious;
>> > which is unseen, beyond empirical dealings, beyond the grasp,
>> uninferable,
...
>> >
>> > In his commentary, aadi sankara points out that the internal world is
>> that
>> > of dream.
>> > The external is that of waking. The awareness of both is in the
>> transition
...
>> >
>> > Once the mind is still, and the heart open - once all attachment has
>> ceased
...
>> >> Then ,what is the need to learn it from vedas or why we have struggle
>> or to
>> >> learn from guru..brahma jgnana..and gain moksha?
>> >>
>> >> 2)What is the difference between moksha and everyday deep sleep?
>> >>
>> >> 3)Once we get moksha then there is no re birth and we always be in
>> moksha
>> >> state or deep sleep state ...
>> >> Even every day I go to deep sleep or moksha state,where there is no
>> jgnana
...
>> >>> that if the whole world is false, then, the Vedas and Upanishads are
>> also
>> >>> false. Then, why should one read the scriptures, which are false
>> anyway?
>> >> To
>> >>> this question, Sri Abhinava Vidya Theertha Mahaswami, who apart from
>> all
...
>> >> teach
>> >>> you reality. So, a study of Vedas and Upanishads will not be a waste
>> and
>> >>> the study will definitely guide you to self-abidance or Atma-Vichara
>> and
...
>> >>> themselves with a name and form, body,gender, nationality, sex, etc.,
>> the
...
>> >>>
>> >>> Ok Sir,                          I will not mix up the two
>> metaphors,now
>> >>> let us take the seeing snake in rope example....When we come to know
>> that
...
>> >>> Similarly after brahma jgnana (Athma jagrathavasta)...there should be
>> the
...
>> >>> different from deep sleep. If waking state is the upamaana that you
>> want
>> >> to
...
>> >>> knowledge, the known and the knower in jaagrath state. Always being
>> alert
>> >>> about the lack of complete identification, one can follow your line of
>> >>> thought. This example has absolutely no bearing on the one the
>> Upanishad
...
>> >> to
>> >>> samsara(jagath)....
>> >>> Similarly...to which state you compare to waken up state?...is it
>> samsara
...
>> >>> observe the assumptions: that you exist separately from Brahman, that
>> the
...
>> >> we
>> >>> should caution that we should not take drishTAntas, (examples),
>> upamaanas
>> >>> (comparisons) etc. literally. If an example or upamaana were completely
>> >>> identical to a given situation then it would cease to be an example or
>> an
>> >>> upamaana.
>> >>> For instance in the context of Indian batsmen, a 6 footer might be
>> >>> considered tall and we might say things like "it is easy for tall
>> batsmen
...
>> >>> NBA." Now if a same person X were to play both games, he is "tall" in
>> one
...
>> >> Moksha
>> >>> and we should not mix it with other examples - Vaidika or otherwise -
>> and
>> >>> get confused.
>> >>> RegardsN. Siva Senani
>> >>>
...
>> >>> Subject: Re: Weather Vedas are considered as false in ultimate reality?
>> >>>
>> >>> For example,                              while explaining dreaming
>> state
...
>> >>> sleep state ,as when we go to deep sleep state dream become
>> false...which
...
>> सह)
>> >>> and cannot be described. However all of reach a similar state on a
>> daily
...
>> >> from
>> >>> this deep sleep state all the time. However the situation while we are
>> in
>> >>> deep sleep is comparable to Moksha.
>> >>> The Upanishad thus describes the state of Moksha, using the state of
>> Deep
...
>> >>> acquiring a fancy car), it only needs to be discovered, like the chain
>> in
>> >>> one's neck that one thought is lost. So, in reality, the AtmA is always
>> >>> Mukta - only we don't understand that. So if is perfectly reasonable
>> that
>> >>> one of our three states is used to describe the state of Moksha.
>> >>> This is also the reason why I quoted BSB 4.1.3 where Bhagavatpada
>> clearly
...
>> >> (prameya).
>> >>> But no two such entities are there in ultimate reality. Only one
>> entity,
>> >>> Brahman, is there. So when we talk of that state, it is untenable to
>> talk
>> >>> of Pramaataa, Prameyaa etc. separately.
>> >>> Therefore, Veda is only an upaaya to know Brahman. Once Brahman is
>> known
>> >>> Veda is non-different from Brahman and hence अवेदः. At no point does
>> >>> Saastrakrit ever say that Veda is false, only its abhaava - absence
>> (that
...
>> >>>
>> >>> Namaste Shiv Sir,
>> >>>                                            In your explanation you
>> said,
>> >>>                                                     ///There the
>> journey
>> >>> of the Self in the states of waking, dreaming, and dreamless sleep are
>> >>> expounded. While dealing with the last, the Upanishad teaches that
>> when a
>> >>> man is embraced by prAj~na (the intelligent Self), then he does not
>> know
>> >>> anything without or within. This state is described in 4.3.22 - “Then a
>> >>> father is not a father, a mother not a mother, the worlds not worlds,
>> the
...
>> >>> Sramana not a Sramana, a Tapasa not a Tapasa. He is not followed by
>> good,
>> >>> not followed by evil, for he has then overcome all the sorrows of the
>> >>> heart.” (Max Muller's translation) (PaulkaSa is the son of a Sudra
>> father
>> >>> and a Kshatriya mother – i.e. a result of pratiloma marriage; Sramana
>> is
>> >> a
>> >>> mendicant).
>> >>>
...
>> >>> Now we are in dreaming state (samsara) where there is (little)
>> knowledge
...
>> >> no?..
>> >>> As when we wake up from deep sleep we have knowledge of vedas
>> back,which
