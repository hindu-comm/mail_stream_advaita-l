+++
title = "200 Srinath Vedagarbha"
date = "2017-03-29"
upstream_url = "https://lists.advaita-vedanta.org/archives/advaita-l/2017-March/044890.html"

+++
[Archive link](https://lists.advaita-vedanta.org/archives/advaita-l/2017-March/044890.html)
...
about shruti is still not addressed. Any pointer from siddhi is appreciated.
...
> Anand
>
> On Mon, Mar 27, 2017 at 9:16 PM, Anand Hudli <anandhudli at hotmail.com>
> wrote:
>
...
> > Light
> > >> emanates from both the rooms, ie. light from the gem in A and light
> from
> > >> the lamp in B. Two persons, P1 and P2 are standing outside the two
> > rooms,
> > >> which are themselves some distance apart so that P1 is somewhere near
> A
> > and
> > >> P2 is near B.  Person P1 sees the light from room A only (not from B)
> > and
> > >> runs after it thinking it is coming from a gem. Person P2 see the
> light
...
> > > light is said to be coming from the gem only; then why would anyone
> call
...
> > > contradiction in terms. If the initial jnAna is later found to be
> samvAdi
...
> > distance and location of the gem is not easily grasped. So here the
> bhrama
...
> > plenty of examples of saMvAdi-bhrama. Also, Shri Yogendranath Bagchi in
> his
...
> > Upon seeing a film of dust, one may mistakenly think it is smoke and
> infer
> > the presence of fire (by the inference rule "where there is smoke there
> is
> > fire" or the vyApti relation between fire and smoke). However, due to
> luck,
...
> > Hence, there is no rule that the object of illusion is sublated (ie.
> there
...
> > inferring fire due to the presence of smoke. The bhrama viShaya is fire
> and
> > it is not sublated even after the bhrama ends.
> >
> > Further examples are found in the PanchadashI as noted above. One may
> > sprinkle on oneself the water of GodAvari thinking it is Gangodaka and
> get
...
> > result from sprinkling Ganga water upon oneself. However, one does
> achieve
...
> >> > adhisTAna of bhrama, but the lamp is. Although gem do exist in the
> room
...
> >> (superimposed) is an objection and the adhisThAna is a
> misunderstanding!!
> >> It looks like an objection (snake), but upon examination it turns out
> to be
...
> >> Light emanates from both the rooms, ie. light from the gem in A and
> light
> >> from the lamp in B. Two persons, P1 and P2 are standing outside the
> >> two rooms, which are themselves some distance apart so that P1 is
> somewhere
> >> near A and P2 is near B.  Person P1 sees the light from room A only (not
> >> from B) and runs after it thinking it is coming from a gem. Person P2
> see
> >> the light from room B only (not from A) and runs after it, also
> thinking it
> >> is coming from a gem. However, P1 does succeed in finding the gem in
> room
> >> A, whereas P2 finds no gem but a lamp in room B. We say person P1
> >> experienced a saMvAdi-bhrama (P1 found the desired object), while
> person P2
...
> >>> definition based on the pramANa-candrikA, Atma-svarUpa-jnAnAdi-
> vyanjakatvam,
> >>> i.e that which reveals the nature of Atman, its properties, cognition,
> etc.
> >>>
...
> >>> argued for sarva-nivRtti earlier. This is necessary because the
> dvaitin can
> >>> and does argue that brahma-jnAna arising from shruti is also sublated.
> This
> >>> objection can be met with using two approaches. First, taking the
> >>> vyavahArika view, shruti is infallible and without any defect (doSha).
> It
> >>> augments other pramANas such as perception and inference, and makes
> known
> >>> what is not known through them, without contradicting them. That is
> why he
> >>> says, "advaita-tAtparya-grahasya ca pratyakShAdyavirodhena
> pramArUpatayA
> >>> doShatva-abhAvAt na tajjanyamadvaitajnAnaM bAdhyam. However, the
> dvaitin
...
> >>> perceiving the light emanating from a room containing a gem, think it
> is
> >>> the gem itself and runs after it and does, in fact, find the gem.
> Another
> >>> person perceiving light emanating from another room containing a lamp
> also
> >>> thinks it is a gem and runs after it, only to discover there is no
> gem. The
> >>> first is a case of samvAdi-bhrama, whereas the second is a case of
> >>> visaMvAdi-bhrama. In the first case, there is doSha-janyatva, arising
> from
