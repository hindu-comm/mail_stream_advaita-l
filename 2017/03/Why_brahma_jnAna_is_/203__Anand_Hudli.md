+++
title = "203 Anand Hudli"
date = "2017-03-31"
upstream_url = "https://lists.advaita-vedanta.org/archives/advaita-l/2017-March/044904.html"

+++
[Archive link](https://lists.advaita-vedanta.org/archives/advaita-l/2017-March/044904.html)
...
>
>
> It is not trouble in understanding, but the concept itself is
contradiction
> in terms.
>
> Setting aside the examples, let's come to the context in which this
example
...
>
> How do anyone is certain this jagat-bhramA is that of samvAdi type? Until
a
...
It is through the examples that the concept emerges, not the other way
round. If the examples make sense, so should the concept. In a
saMvAdi-bhrama, there has to be a bhrama component, else it would not
qualify to be a  bhrama. In addition, the desired/anticipated result,
phala, should not be sublated after the bhrama ends. For example, in the
typical silver-shell illusion, the desired result during the illusion is
silver. But the silver gets sublated upon termination of the illusion. So
this is not a saMvAdi bhrama.

Your other question has to do with the validity of shruti itself. While all
pramANas lose their status in the pAramArthika level, they are considered
to be valid in the vyAvahArika level. So the shruti too has to be accepted
as valid, infallible, without a defect, and capable of yielding the desired
result as stated in the shruti itself. This also means we have to accept
the bhramas associated with shruti, including upAsanas, etc., as
saMvAdi-bhramas.

Madhusudana has quoted Sureshvara's vArtika: ato avabodhakatvena
duShTakAraNavarjanAt| abAdhAcca pramANatvaM vastunyakShAdivat shruteH| Just
as Perception, etc are considered to be valid (in vyvahAra) due to their
making something unknown known (ajnAta-jnApaka), being free of defects, and
not being sublated (in vyavahAra), so too the shruti is a pramANa (for
brahma-jnAna).

Anand


On Tue, Mar 28, 2017 at 5:02 PM, Anand Hudli <anandhudli at hotmail.com> wrote:
...
>> infer the presence of fire (by the inference rule "where there is smoke
>> there is fire" or the vyApti relation between fire and smoke). However, due
>> to luck, there may actually be fire in the place where it is expected. This
>> is an exception to the (proposed) rule that an object of illusion is
>> sublated. Hence, there is no rule that the object of illusion is sublated
>> (ie. there is no rule that there is viShaya-bAdha due to bhramajanyatva).
>> Note here that the (saMvAdi) bhrama consists of superimposing smoke on dust
>> *and* inferring fire due to the presence of smoke. The bhrama viShaya is
>> fire and it is not sublated even after the bhrama ends.
>>
